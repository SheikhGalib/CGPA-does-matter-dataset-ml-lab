...
\captionof{table}{KUET semester 7 by submission date.}\label{tab:gapdates}
\begin{tabular}{|l|r|r|r|r|}
\hline
\rowcolor{hdr}\hd{Date} & \hd{n} & \hd{Big} & \hd{\%} & \hd{Removed} \\ \hline
\tabinput{tables/gap_kuet_dates.tex}
\end{tabular}
\end{minipage}
\end{table}
...
\begin{figure}[!htbp]
\centering
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{figures/deck/sgpa_x_cgpa_aug67.png}\caption{6--7 Aug KUET semester 7 (455 rows)}\end{subfigure}\hfill
\begin{subfigure}{0.49\linewidth}\includegraphics[width=\linewidth]{figures/deck/sgpa_x_cgpa_rest.png}\caption{All other students (647 rows)}\end{subfigure}
\caption{Recent SGPA group (rows) against CGPA group (columns). In the checked batch the answers are spread over every cell, as random choices would be. For everyone else most answers lie on or next to the diagonal.}
\label{fig:scatter}
\end{figure}
